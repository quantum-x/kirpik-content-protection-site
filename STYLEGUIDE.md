...
- **Script-app pages carry a claim-by-claim rebuttal.** "Does it prevent content theft?" opens with the categorical paragraph (does not stop the machine-readable feeds JSON/XML/ATOM; easily bypassed and easily disabled; friction lands on legitimate customers, none on scrapers and thieves), then "Its claims, one by one:" quoting every listing claim with the technical fact that answers it (`rebutClaim` in compare-data.ts; the build fails on any unclassified claim). Facts do the rebuking: never "scam/fraud/lie", no opinion adjectives.
- **The non-disclosure fact is stated, pithily:** after the claim list, "{App}'s listing does not state these limits." The three listings that themselves use deterrence-only language (CopyBlock, Defendify, Cozy) instead get "Its listing frames this as deterrence rather than prevention." Developer admissions from public review replies may be quoted verbatim (Dropship Protect, Webyze).
- Comparison-page site-check band sub (fixed template): `Do you use <strong>{App}</strong> or any other "Content Protection" App? Enter your Shopify store address to see if your content is actually protected.`
